...
|   output_directory\
|   └── DAC_Documents\
|       └── library_info_<GGG_PARENT><GG_PARENT><G_PARENT>_<YYYYMMDD>.xlsx
*------------------------------------------------------------------*
...
%macro library_info_cli;
    ...
    %let out_file = &doc_dir\library_info_%sysfunc(compress(&ggg_parent,,ka))%sysfunc(compress(&gg_parent,,ka))%sysfunc(compress(&g_parent,,ka))_%sysfunc(today(),yymmddn8.).xlsx;
