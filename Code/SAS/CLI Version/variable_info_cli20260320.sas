...
%macro variable_info_cli;
    ...
    %let out_file = &doc_dir\variable_info_%sysfunc(compress(&ggg_parent,'_',kad))_%sysfunc(compress(&gg_parent,'_',kad))_%sysfunc(compress(&g_parent,'_',kad))_%sysfunc(today(),yymmddn8.).xlsx;
